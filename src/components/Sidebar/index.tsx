...
const AccountsItems: Account[] = [
  {
    accountCurrency: "Brazil",
...
interface GroupedAccounts {
  accountsByCurrency: { [key: string]: Account[] };
  investing: Account[];
  cashSavings: Account[];
}

const groupAccounts = (accounts: Account[]): GroupedAccounts => {
  ...
  accounts.forEach(account => {
    const { accountCurrency, accountType } = account;

    if (accountType === 'Investing') {
      grouped.investing.push(account);
    } else if (accountType === 'Cash Savings') {
      grouped.cashSavings.push(account);
    } else {
      if (!grouped.accountsByCurrency[accountCurrency]) {
        grouped.accountsByCurrency[accountCurrency] = [];
      }
      grouped.accountsByCurrency[accountCurrency].push(account);
    }
  });
...
const groupedAccounts = groupAccounts(AccountsItems);
...
const Sidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  ...
  const totalBalance = AccountsItems.reduce((total, account) => total + account.balance, 0);



// IMPLEMENTING CODE FOR ACCOUNTS IN THE SIDEBAR
...
const formatCurrency = (currency: string, amount: number): string => {
  const currencyCode = currencyCodeMap[currency] || 'USD'; // Default to USD if not found
  const options = { style: 'currency', currency: currencyCode, minimumFractionDigits: 2, maximumFractionDigits: 2 };
  return new Intl.NumberFormat('en-US', options).format(amount);
};

const totalBalanceUSD = formatCurrency('USD', totalBalance);
...
const renderGroup = (title: string, accounts: Account[]) => {
  ...
  return (
    <SidebarLinkGroup
      key={title}
      activeCondition={pathname.includes(title.toLowerCase().replace(/\s+/g, '-'))}
    >
      {(handleClick, open) => (
        <React.Fragment>
          <Link
            href="#"
            className={`group relative flex justify-between uppercase items-center mr-[-2rem] rounded-sm px-4 py-2 font-medium text-bodydark1 duration-300 ease-in-out bg-slate-900/60 dark:bg-[#1a222c76] hover:bg-graydark dark:hover:bg-meta-4 ${
              pathname.includes(title.toLowerCase().replace(/\s+/g, '-')) && "bg-graydark dark:bg-meta-4"
            }`}
            onClick={(e) => {
              e.preventDefault();
          ...
          >
           ...
           <div className="">
           <span className="mr-6">{formatCurrency(accounts[0]?.accountCurrency || 'USD', totalBalance)}</span>
           
            <svg
              className={`absolute mr-1 right-4 top-1/2 -translate-y-1/2 fill-current ${open && "rotate-180"}`}
              width="20"
              height="20"
              viewBox="0 0 20 20"
              fill="none"
          ...
          <div className={`translate transform overflow-hidden ${!open && "hidden"}`}>
            <ul className="mb-5.5 mt-4 flex flex-col gap-2.5 pl-6">
              {accounts.map((account, idx) => (
                ...
                <span className={` ${account.balance > 0 ? "group-hover:text-white" : "text-danger group-hover:text-rose-600"}`}>
                {formatCurrency(account.accountCurrency, account.balance)}
                </span>
              </li>
  ...
  return (
    <aside
      ref={sidebar}
      className={`absolute left-0 top-0 z-9999 flex h-screen w-[320px] flex-col overflow-y-hidden bg-black duration-300 ease-linear dark:bg-boxdark lg:static lg:translate-x-0 ${
    ...
    >
      ...
      <div className="no-scrollbar flex flex-col overflow-y-auto duration-300 ease-linear">
        {/* <!-- Sidebar Menu --> */}
        <nav className="mt-5 px-4 py-1 lg:mt-4 lg:px-6">
          {/* <!-- Menu Group --> */}
          <div>
            <h3 className="mb-4 ml-4 text-sm font-bold text-[#3C50E0]">
              ACCOUNTS
            </h3>
                ...
                {(handleClick, open) => {
                  return (
                    <React.Fragment>
                      <Link
                        href="#"
                        className={`group relative  justify-between  mr-[-2rem] flex items-center rounded-sm px-4 py-2 font-bold text-[#80CAEE] duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 ${
                          (pathname === "/dashboard" ||
                      ...
                      >
                        ...
                        <div className="mr-2">
                          <span className="">{totalBalanceUSD}</span>
                          {/* <svg
              ...
              <>
                {Object.keys(groupedAccounts.accountsByCurrency).map(
                  (currency) =>
                    renderGroup(
                      currency,
                      groupedAccounts.accountsByCurrency[currency],
                    ),
                )}
                {renderGroup("INVESTMENTS", groupedAccounts.investing)}
                {renderGroup("CASH", groupedAccounts.cashSavings)}
              </>
